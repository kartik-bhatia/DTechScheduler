...
public class CurriculumCoursePanel extends SolutionPanel {
    ...
    private final TimeTablePanel<Student, Period> studentPanel;
    ...
    public CurriculumCoursePanel() {
        ...
        studentPanel = new TimeTablePanel<Student, Period>();
        tabbedPane.add("Students", new JScrollPane(studentPanel));
    ...
    private void defineGrid(CourseSchedule courseSchedule) {
        ...
        studentPanel.defineColumnHeaderByKey(HEADER_COLUMN_GROUP1); // Day header
        studentPanel.defineColumnHeaderByKey(HEADER_COLUMN); // Period header
        for (Student student : courseSchedule.getStudentList()) {
            studentPanel.defineColumnHeader(student, footprintWidth);
        }
        ...
        studentPanel.defineRowHeaderByKey(HEADER_ROW); // Student header
        ...
        for (Period period : courseSchedule.getPeriodList()) {
            ...
            studentPanel.defineRowHeader(period);
        }
        ...
        studentPanel.defineRowHeader(null); // Unassigned period
    }
    ...
    private void fillCells(CourseSchedule courseSchedule) {
        ...
        studentPanel.addCornerHeader(HEADER_COLUMN_GROUP1, HEADER_ROW, createHeaderPanel(new JLabel("Day")));
        studentPanel.addCornerHeader(HEADER_COLUMN, HEADER_ROW, createHeaderPanel(new JLabel("Time")));
        fillStudentCells(courseSchedule);
    ...
    private void fillStudentCells(CourseSchedule courseSchedule) {
        for (Student student : courseSchedule.getStudentList()) {
            studentPanel.addColumnHeader(student, HEADER_ROW,
                    createHeaderPanel(new JLabel(student.getLabel(), SwingConstants.CENTER)));
        }
    }
    ...
    private void fillDayCells(CourseSchedule courseSchedule) {
        for (Day day : courseSchedule.getDayList()) {
            ...
            studentPanel.addRowHeader(HEADER_COLUMN_GROUP1, dayStartPeriod, HEADER_COLUMN_GROUP1, dayEndPeriod,
                    createHeaderPanel(new JLabel(day.getLabel())));
            ...
            for (Period period : day.getPeriodList()) {
                ...
                studentPanel.addRowHeader(HEADER_COLUMN, period,
                        createHeaderPanel(new JLabel(period.getTimeslot().getLabel())));
        ...
        studentPanel.addRowHeader(HEADER_COLUMN_GROUP1, null, HEADER_COLUMN, null,
                createHeaderPanel(new JLabel("Unassigned")));
    ...
    private void fillLectureCells(CourseSchedule courseSchedule) {
        ...
        for (Lecture lecture : courseSchedule.getLectureList()) {
            ...
            for (Student student : lecture.getStudentList()) {
                studentPanel.addCell(student, lecture.getPeriod(),
                        createButton(lecture, lectureColor));
            }
